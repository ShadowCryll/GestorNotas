...
def calcular_promedio_cursos(cursos):
    # Precondición:
    #   cursos es una lista de tuplas (curso, nota)
    # Postcondición:
...
def promedio_general_grupo(estudiantes):
    # Precondición:
    #   estudiantes contiene registros válidos
    # Postcondición:
    #   Retorna el promedio general del grupo (promedio de promedios)
    if not estudiantes:
        return 0
    total = sum(calcular_promedio_cursos(est["cursos"]) for est in estudiantes)
...
def ordenar_burbuja(estudiantes):
    ...
    #   estudiantes contiene registros válidos
    # Postcondición:
    #   Retorna una lista ordenada por promedio descendente
...
def registrar_estudiante(estudiantes, historial):
    ...
    # Postcondición:
    #   Agrega un nuevo estudiante y registra la acción
    nombre = input("Ingrese el nombre del estudiante: ").strip()

    # Evitar duplicados
    for est in estudiantes:
        if est["nombre"].lower() == nombre.lower():
            print("Ya existe un estudiante con ese nombre.")
            return
...
def mostrar_estudiantes(estudiantes):
    ...
    # Postcondición:
    #   Muestra estudiantes, cursos y promedio
    if not estudiantes:
    ...
    prom_grupo = promedio_general_grupo(estudiantes)
    print(f"\n Promedio general del grupo: {prom_grupo:.2f}")
...
def contar_aprobados(estudiantes):
    ...
    #   estudiantes contiene registros válidos
    # Postcondición:
    #   Muestra quiénes aprobaron o reprobaron según su promedio
    if not estudiantes:
        print("No hay estudiantes registrados.")
        return
...
def actualizar_nota(estudiantes, historial):
    ...
    # Postcondición:
    #   Actualiza una nota y registra la acción
...
def eliminar_curso(estudiantes, historial):
    ...
    #   estudiantes contiene registros
    # Postcondición:
    #   Elimina un curso tras confirmación
    nombre = input("Ingrese el nombre del estudiante: ").strip().lower()
    for est in estudiantes:
        if est["nombre"].lower() == nombre:
            for i, (curso, nota) in enumerate(est["cursos"], start=1):
            ...
            if 0 <= indice < len(est["cursos"]):
                curso_eliminar = est["cursos"][indice][0]
                confirmar = input(f"¿Seguro que deseas eliminar '{curso_eliminar}'? (S/N): ").strip().upper()
                if confirmar == "S":
...
def eliminar_estudiante(estudiantes, historial):
    # Precondición:
    #   estudiantes contiene registros válidos
    # Postcondición:
    #   Elimina completamente un estudiante de la lista
    nombre = input("Ingrese el nombre del estudiante a eliminar: ").strip().lower()
    for i, est in enumerate(estudiantes):
        if est["nombre"].lower() == nombre:
            confirmar = input(f"¿Seguro que deseas eliminar al estudiante '{est['nombre']}'? (S/N): ").strip().upper()
            if confirmar == "S":
                eliminado = estudiantes.pop(i)
                historial.append(f"Eliminó completamente a {eliminado['nombre']}")
                print(f"Estudiante '{eliminado['nombre']}' eliminado del sistema.")
            else:
                print("Operación cancelada.")
            return
    print("Estudiante no encontrado.")
...
def agregar_a_cola_revision(cola, estudiantes):
    ...
    #   estudiantes contiene registros válidos
    # Postcondición:
    #   Agrega un estudiante a la cola (FIFO)
...
def procesar_cola_revision(cola):
    # Precondición:
    #   cola contiene estudiantes
    # Postcondición:
    #   Procesa al primer estudiante en orden FIFO
    if not cola:
        print("No hay estudiantes en la cola de revisión.")
    else:
        est = cola.pop(0)
        print(f"\nRevisando estudiante: {est['nombre']}")
        for curso, nota in est["cursos"]:
            print(f"   {curso}: {nota}")
        promedio = calcular_promedio_cursos(est["cursos"])
        print(f"Promedio general: {promedio:.2f}")
...
def mostrar_menu():
    ...
    print("7. Ordenar estudiantes (Burbuja)")
    print("8. Ordenar estudiantes (Inserción)")
    print("9. Ver historial (Pila)")
    print("10. Agregar estudiante a revisión (Cola)")
    print("11. Procesar revisión (Cola)")
    print("12. Salir")
...
def main():
    ...
    while opcion != 12:
        ...
        if opcion == 1:
            ...
            eliminar_estudiante(estudiantes, historial_acciones)
        elif opcion == 7:
            ...
            print("Gracias por usar el Gestor de Notas (Avance 06).")
        else:
